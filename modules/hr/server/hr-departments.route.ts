...
import { CreateDepartmentDTO, GetDepartmentsQuery } from '@kuraykaraaslan/hr/server/hr.dto'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    const query = GetDepartmentsQuery.parse({
      page: sp.get('page') ? Number(sp.get('page')) : 0,
      pageSize: sp.get('pageSize') ? Number(sp.get('pageSize')) : 20,
      search: sp.get('search') ?? undefined,
    })
    const { data, total } = await DepartmentService.list(tenantId, query)
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    const dto = CreateDepartmentDTO.parse(await request.json())
    const item = await DepartmentService.create(tenantId, dto)
