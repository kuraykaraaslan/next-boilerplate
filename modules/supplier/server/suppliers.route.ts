...
import { CreateSupplierDTO, GetSuppliersQuery } from '@kuraykaraaslan/supplier/server/supplier.dto'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    const query = GetSuppliersQuery.parse({
      page: sp.get('page') ? Number(sp.get('page')) : 0,
      pageSize: sp.get('pageSize') ? Number(sp.get('pageSize')) : 20,
      search: sp.get('search') ?? undefined,
    })
    const { data, total } = await SupplierService.list(tenantId, query)
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    const dto = CreateSupplierDTO.parse(await request.json())
    const item = await SupplierService.create(tenantId, dto)
