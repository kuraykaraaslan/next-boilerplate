...
import { CreateCarrierDTO, GetCarriersQuery } from '@kuraykaraaslan/order_fulfillment/server/order_fulfillment.dto'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    const query = GetCarriersQuery.parse({
      page: sp.get('page') ?? undefined,
      pageSize: sp.get('pageSize') ?? undefined,
      search: sp.get('search') ?? undefined,
    })
    const { data, total } = await OrderFulfillmentCarrierConfigService.list(tenantId, query)
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    const dto = CreateCarrierDTO.parse(await request.json())
    const item = await OrderFulfillmentCarrierConfigService.create(tenantId, dto)
