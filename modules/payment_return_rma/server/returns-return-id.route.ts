...
import { UpdateReturnDTO } from '@kuraykaraaslan/payment_return_rma/server/payment_return_rma.dto'
...
export async function GET(request: NextRequest, { params }: Ctx) {
  ...
  try {
    const item = await PaymentReturnRmaService.getById(tenantId, returnId)
...
export async function PATCH(request: NextRequest, { params }: Ctx) {
  ...
  try { await auth(request, tenantId) } catch (e: any) { return NextResponse.json({ message: e.message }, { status: 403 }) }
  try {
    const dto = UpdateReturnDTO.parse(await request.json())
    const item = await PaymentReturnRmaService.update(tenantId, returnId, dto)
    return NextResponse.json({ item })
