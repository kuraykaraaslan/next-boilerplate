...
import { RefundReturnDTO } from '@kuraykaraaslan/payment_return_rma/server/payment_return_rma.dto'
...
export async function POST(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const dto = RefundReturnDTO.parse(body ?? {})
    const item = await PaymentReturnRmaService.refund(tenantId, returnId, dto)
