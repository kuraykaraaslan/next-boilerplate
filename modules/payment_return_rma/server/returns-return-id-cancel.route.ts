...
import { ModerateReturnDTO } from '@kuraykaraaslan/payment_return_rma/server/payment_return_rma.dto'
...
export async function POST(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const dto = ModerateReturnDTO.parse(body ?? {})
    const item = await PaymentReturnRmaService.cancel(tenantId, returnId, dto.note)
