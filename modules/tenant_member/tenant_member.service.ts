...
export default class TenantMemberService {
  ...
  static async resolveDefaultRole(tenantId: string): Promise<TenantMemberRole> {
  ...
  static async create(data: CreateTenantMemberInput): Promise<SafeTenantMember> {
    ...
    const memberRole = data.memberRole ?? (await TenantMemberService.resolveDefaultRole(data.tenantId));
  ...
  static async unsuspend(tenantMemberId: string, tenantId: string): Promise<SafeTenantMember> {
    const ds = await tenantDataSourceFor(tenantId);
    const repo = ds.getRepository(TenantMemberEntity);
    const member = await repo.findOne({ where: { tenantMemberId, tenantId, deletedAt: IsNull() } });
    if (!member) throw new AppError(TenantMemberMessages.MEMBER_NOT_FOUND, 404, ErrorCode.NOT_FOUND);
  ...
  static async touchLastActive(tenantId: string, userId: string): Promise<void> {
    const ds = await tenantDataSourceFor(tenantId);
   ...
   */
  static async transferOwnership(tenantId: string, toUserId: string, demoteToRole: TenantMemberRole = 'ADMIN'): Promise<void> {
    const ds = await tenantDataSourceFor(tenantId);
    await ds.transaction(async (mgr) => {
