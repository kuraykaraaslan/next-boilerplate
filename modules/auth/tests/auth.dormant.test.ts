...
vi.mock('@/modules/env', () => ({ env: { NODE_ENV: 'test' } }));
...
vi.mock('@/modules/tenant/tenant.constants', () => ({ ROOT_TENANT_ID: 'root' }));
vi.mock('../../user/user.service', () => ({ default: { getByEmail: vi.fn(async () => null), invalidate: vi.fn(async () => {}) } }));
...
vi.mock('../../user_security/user_security.service', () => ({ default: { pushPasswordHistory: vi.fn(async () => {}) } }));
vi.mock('../../audit_log/audit_log.service', () => ({ default: { log: vi.fn(async () => {}) } }));
vi.mock('../../audit_log/audit_log.enums', () => ({ AuditActions: { AUTH_DORMANT_DISABLED: 'd' } }));
