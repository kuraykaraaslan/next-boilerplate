...
export default class TenantOnboardingService {
  ...
  private static async isDnsConfigured(tenantId: string): Promise<boolean> {
    try {
      const { default: TenantDomainService } = await import('@/modules/tenant_domain/tenant_domain.service');
  ...
  private static async hasMembers(tenantId: string): Promise<boolean> {
    try {
      const { default: TenantMemberService } = await import('@/modules/tenant_member/tenant_member.service');
