...
import { tenantDataSourceFor, getDataSource, clearTenantDsCache } from '@/modules/db';
import { Tenant } from './entities/tenant.entity';
import Logger from '@/modules/logger';
import TenantMessages from './tenant.messages';
import { AppError, ErrorCode } from '@/modules/common/app-error';
import { clearTenantCache } from '@/modules/redis';
...
export default class TenantDeletionService {
  ...
  private static async cascadePurge(tenantId: string): Promise<void> {
    ...
    try {
      const { default: ApiKeyService } = await import('@/modules/api_key/api_key.service');
