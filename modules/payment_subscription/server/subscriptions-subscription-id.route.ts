...
import { UpdateSubscriptionDTO } from '@kuraykaraaslan/payment_subscription/server/payment_subscription.dto'
...
export async function GET(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const withPlan = sp.get('withPlan') === 'true'
    const item = await PaymentSubscriptionService.getSubscription(tenantId, subscriptionId, withPlan)
...
export async function PATCH(request: NextRequest, { params }: Ctx) {
  ...
  try { await auth(request, tenantId) } catch (e: any) { return NextResponse.json({ message: e.message }, { status: 403 }) }
  try {
    const dto = UpdateSubscriptionDTO.parse(await request.json())
    const item = await PaymentSubscriptionService.updateSubscription(tenantId, subscriptionId, dto)
    return NextResponse.json({ item })
...
export async function DELETE(request: NextRequest, { params }: Ctx) {
  ...
  try { await auth(request, tenantId) } catch (e: any) { return NextResponse.json({ message: e.message }, { status: 403 }) }
  try {
    await PaymentSubscriptionService.deleteSubscription(tenantId, subscriptionId)
