...
import { CancelSubscriptionDTO } from '@kuraykaraaslan/payment_subscription/server/payment_subscription.dto'
...
export async function POST(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const dto = CancelSubscriptionDTO.parse(body ?? {})
    const item = await PaymentSubscriptionService.cancelSubscription(tenantId, subscriptionId, dto)
