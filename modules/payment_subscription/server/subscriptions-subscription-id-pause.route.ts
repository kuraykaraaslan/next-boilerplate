...
import { PauseSubscriptionDTO } from '@kuraykaraaslan/payment_subscription/server/payment_subscription.dto'
...
export async function POST(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const dto = PauseSubscriptionDTO.parse(body ?? {})
    const item = await PaymentSubscriptionService.pauseSubscription(tenantId, subscriptionId, dto)
