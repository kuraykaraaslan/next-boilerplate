...
vi.mock('@/modules/env', () => ({ env: { REDIS_URL: 'redis://test', NODE_ENV: 'test' } }));
vi.mock('@/modules/redis', () => ({ default: { publish: vi.fn(async () => 0) } }));
vi.mock('@/modules/db', () => ({ tenantDataSourceFor: vi.fn() }));
vi.mock('@/modules/setting/setting.service', () => ({ default: { getByKeys: vi.fn(async () => ({})) } }));
