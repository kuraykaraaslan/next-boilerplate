...
import { CreateOrderDTO, GetOrdersQuery } from '@kuraykaraaslan/order/server/order.dto'
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    const dto = CreateOrderDTO.parse(await request.json())
    const item = await OrderService.create(tenantId, dto)
