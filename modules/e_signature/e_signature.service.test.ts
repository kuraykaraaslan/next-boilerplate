...
vi.mock('@/modules/redis', () => ({
  default: {
    ...
    setex: vi.fn(async (key: string, _ttl: number, value: string) => { redisStore.set(key, value); return 'OK'; }),
    del: vi.fn(async (...keys: string[]) => { keys.forEach((k) => redisStore.delete(k)); return keys.length; }),
    incr: vi.fn(),
    expire: vi.fn(),
    scanStream: vi.fn(),
    ping: vi.fn(),
