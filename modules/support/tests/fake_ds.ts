...
/**
 * Minimal in-memory DataSource good enough for the back-office access patterns:
 * findOne/find/findAndCount with simple equality + `In(...)` where clauses and
 * single/multi-key ordering, save/create, a transaction passthrough, the tiny
 * subset of the query builder `allocateTicketNumber` uses, and a no-op
 * pessimistic lock. Mirrors the wallet test's fake DataSource.
 */

const PK: Record<string, string> = {
  ApprovalQueueItem: 'approvalItemId',
  SupportTicket: 'ticketId',
  SupportTicketMessage: 'ticketMessageId',
};
...
function makeRepo(store: Record<string, Record<string, unknown>[]>, name: string, tick: () => number) {
  ...
  return {
    ...
    save: (obj: Record<string, unknown>) => {
      ...
      if ((name === 'ApprovalQueueItem' || name === 'SupportTicket') && obj['updatedAt'] === undefined) {
    ...
    // Tiny query-builder shim used only by allocateTicketNumber.
    createQueryBuilder: () => {
      const state: { tenantId?: string; prefix?: string } = {};
      const qb = {
        select: () => qb,
        where: (_clause: string, p?: Record<string, unknown>) => {
          if (p && 'tid' in p) state.tenantId = p['tid'] as string;
          return qb;
        },
        andWhere: (_clause: string, p?: Record<string, unknown>) => {
          if (p && 'prefix' in p) state.prefix = String(p['prefix']).replace(/%$/, '');
          return qb;
        },
        orderBy: () => qb,
        addOrderBy: () => qb,
        ...
        getRawOne: <T>() => {
          const rows = arr().filter(
            (r) =>
              r['tenantId'] === state.tenantId &&
              typeof r['ticketNumber'] === 'string' &&
              (r['ticketNumber'] as string).startsWith(state.prefix ?? ''),
          );
          rows.sort((a, b) => {
            const an = parseInt((a['ticketNumber'] as string).split('-').pop() ?? '0', 10);
            const bn = parseInt((b['ticketNumber'] as string).split('-').pop() ?? '0', 10);
            return bn - an;
          });
          return Promise.resolve((rows[0] ? { ticketNumber: rows[0]['ticketNumber'] } : undefined) as T);
        },
      };
      return qb;
    },
...
export function makeFakeDs(): FakeDs {
  ...
  return {
    ...
    transaction: async (cb) =>
      cb({
        // The transaction manager shares the same store; also exposes `query`
        // (no-op advisory lock) via a Proxy-free wrapper below.
        getRepository,
      }),
  } as FakeDs;
}
