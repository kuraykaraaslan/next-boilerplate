// Chorus Pro / Factur-X (FR) e-invoicing adapter — sandboxed. Generates real
// EN 16931-aligned UN/CEFACT CII (the XML at the heart of Factur-X, the same
// syntax as ZUGFeRD 2.x), signs it host-side via host.crypto.signXml (the tenant
// seal key never enters the isolate), and — when a Chorus Pro gateway is
// configured — POSTs it. With no gateway it returns status 'noop' (the document
// is still generated and returned in `raw` for PDF/A-3 embedding). B2G is
// mandatory in France via Chorus Pro; the B2B mandate is rolling out. Ported
// faithfully from the built-in invoice/cii_xml + invoice_fr_choruspro adapter.
...
function buildCiiInvoiceXml(params) {
  ...
  const profile = PROFILE_URN[params.profile] ? params.profile : 'EXTENDED';
...
function sellerParty(s) {
  ...
  return {
    ...
    countryCode: (s.companyCountryCode || 'FR').toUpperCase(),
  };
}
...
globalThis.__plugin = {
  providers: {
    'invoice:adapter': {
      // France is "configured" only once a Chorus Pro gateway endpoint is set —
      // the document can always be built, but B2G transmission requires the API.
      isConfigured: async (_input, host) => Boolean(await host.settings.get('chorusProGatewayUrl')),

      submit: async ({ invoice, lines, seller }, host) => {
        const gatewayUrl = await host.settings.get('chorusProGatewayUrl');
        const profile = (await host.settings.get('chorusProProfile')) || 'EXTENDED';
        ...
        const res = await host.http.fetch(gatewayUrl, {
          method: 'POST',
          headers: { 'content-type': 'application/xml', authorization: 'Bearer {{secret:chorusProToken}}' },
          body: xml,
        });
        const text = String(res.body == null ? '' : res.body);
        if (res.status >= 400) throw new Error('Chorus Pro ' + res.status + ': ' + text.slice(0, 500));

        let externalId;
        try { const j = JSON.parse(text); externalId = j.idFacture || j.id; } catch (_e) { /* non-JSON */ }
        return { externalId, status: 'submitted', raw: { gateway: gatewayUrl, response: text.slice(0, 1000) } };
      },

      // Cancellation in France is an "avoir" (credit note, TypeCode 381) on our
      // side — there is no transmission to undo here.
      cancel: async () => null,
    },
  },
};
