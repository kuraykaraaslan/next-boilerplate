// @deepseek/deepseek — DeepSeek provider plugin (OpenAI-compatible). Sandboxed;
// egress locked to api.deepseek.com. The API key is injected host-side via the
// {{secret:apiKey}} placeholder (signed egress) — the isolate never sees it.
  ...
  providers: {
    'ai:provider': {
      ...
      chat: async (opts, host) => {
        ...
        const res = await host.http.fetch('https://api.deepseek.com/v1/chat/completions', {
          method: 'POST',
        ...
        if (res.status >= 400) throw new Error('deepseek api ' + res.status + ': ' + String(res.body).slice(0, 300));
      ...
      embed: async () => { throw new Error('deepseek does not support embeddings'); },
    },
  },
};
