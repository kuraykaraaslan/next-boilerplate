...
import { CreateInventoryCountDTO, GetInventoryCountsQuery } from '@kuraykaraaslan/inventory/server/inventory.dto'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    const query = GetInventoryCountsQuery.parse({
      page: sp.get('page') ? Number(sp.get('page')) : 0,
      pageSize: sp.get('pageSize') ? Number(sp.get('pageSize')) : 20,
    })
    const { data, total } = await InventoryCountService.list(tenantId, query)
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    const dto = CreateInventoryCountDTO.parse(await request.json())
    const item = await InventoryCountService.create(tenantId, dto)
