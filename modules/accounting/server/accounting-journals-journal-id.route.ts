...
import { UpdateJournalDTO } from '@kuraykaraaslan/accounting/server/accounting.dto'

type Ctx = { params: Promise<{ tenantId: string; journalId: string }> }
...
export async function PATCH(request: NextRequest, { params }: Ctx) {
  ...
  try { await auth(request, tenantId) } catch (e: any) { return NextResponse.json({ message: e.message }, { status: 403 }) }
  try {
    const dto = UpdateJournalDTO.parse(await request.json())
    const item = await JournalService.update(tenantId, journalId, dto)
    return NextResponse.json({ item })
...
export async function DELETE(request: NextRequest, { params }: Ctx) {
  ...
  try { await auth(request, tenantId) } catch (e: any) { return NextResponse.json({ message: e.message }, { status: 403 }) }
  try {
    await JournalService.delete(tenantId, journalId)
