...
import { CreateFiscalPeriodDTO, GetFiscalPeriodsQuery } from '@kuraykaraaslan/accounting/server/accounting.dto'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    const query = GetFiscalPeriodsQuery.parse({
      page: sp.get('page') ? Number(sp.get('page')) : 0,
      pageSize: sp.get('pageSize') ? Number(sp.get('pageSize')) : 20,
      search: sp.get('search') ?? undefined,
    })
    const { data, total } = await FiscalPeriodService.list(tenantId, query)
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    const dto = CreateFiscalPeriodDTO.parse(await request.json())
    const item = await FiscalPeriodService.create(tenantId, dto)
