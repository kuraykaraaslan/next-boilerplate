...
import { CreateJournalLineDTO } from '@kuraykaraaslan/accounting/server/accounting.dto'
...
export async function GET(request: NextRequest, { params }: Ctx) {
  ...
  try {
    const { data, total } = await JournalLineService.listByParent(tenantId, entryId)
...
export async function POST(request: NextRequest, { params }: Ctx) {
  ...
  try { await auth(request, tenantId) } catch (e: any) { return NextResponse.json({ message: e.message }, { status: 403 }) }
  try {
    const dto = CreateJournalLineDTO.parse(await request.json())
    const item = await JournalLineService.addLine(tenantId, entryId, dto)
