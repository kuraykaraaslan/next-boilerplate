...
export default function SupportPage({ params }: { params: Promise<{ tenantId: string }> }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    setFetchError('');
    try {
      const res = await api.get(`/tenant/${tenantId}/api/back-office/support/tickets`, {
  ...
  const openDetail = useCallback(async (ticketId: string) => {
    ...
    setInternal(false);
    try {
      const res = await api.get(`/tenant/${tenantId}/api/back-office/support/tickets/${ticketId}`);
  ...
  async function sendReply() {
    ...
    setBusy(true);
    try {
      await api.post(`/tenant/${tenantId}/api/back-office/support/tickets/${detail.ticketId}/messages`, {
        body: reply,
        internal,
      });
  ...
  async function patch(action: string, extra: Record<string, unknown> = {}) {
    ...
    setBusy(true);
    try {
      await api.patch(`/tenant/${tenantId}/api/back-office/support/tickets/${detail.ticketId}`, { action, ...extra });
