...
export default function CreateTenantPage({ params }: { params: Promise<{ tenantId: string }> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setApiError('');
    try {
      // Platform-level operation: only available under the root tenant's
      // /admin/tenants/create route, guarded by authenticateAdminRequest.
      await api.post(`/tenant/${tenantId}/api/tenants/create`, values);
      setSuccess(true);
    } catch (err: any) {
  ...
  return (
    <div className="w-full max-w-md space-y-6">
      ...
      <p className="text-center text-sm text-text-secondary">
        <a href={`/tenant/${tenantId}/auth/select-tenant`} className="text-primary font-medium hover:underline">
