...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  try {
    const page = await DynamicPageService.getPageBySlug(tenantId, slug)
    if (page.status !== 'PUBLISHED') return {}

    // Apply translation title/description if lang requested
...
export default async function PublicDynamicPage({ params, searchParams }: Props) {
  ...
  // Check scheduling (startDate / endDate from metadata or first popup block)
  const now = new Date()
  ...
  // Load translation sections if requested
