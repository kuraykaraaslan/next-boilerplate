...
import { UpdateCategoryDTO } from '@nb/blog/server/blog.dto'
...
export async function GET(request: NextRequest, { params }: Ctx) {
  ...
  try {
    const category = await BlogCategoryService.getById(tenantId, categoryId)
...
export async function PATCH(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const category = await BlogCategoryService.update(tenantId, categoryId, dto)
    return NextResponse.json({ category })
...
export async function DELETE(request: NextRequest, { params }: Ctx) {
  ...
  try { await auth(request, tenantId) } catch (e: any) { return NextResponse.json({ message: e.message }, { status: 403 }) }
  try {
    await BlogCategoryService.delete(tenantId, categoryId)
