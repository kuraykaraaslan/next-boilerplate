...
import { UpdatePostDTO } from '@nb/blog/server/blog.dto'
...
export async function GET(request: NextRequest, { params }: Ctx) {
  ...
  try {
    const post = await BlogPostService.getById(tenantId, postId)
...
export async function PATCH(request: NextRequest, { params }: Ctx) {
  ...
  try { await auth(request, tenantId) } catch (e: any) { return NextResponse.json({ message: e.message }, { status: 403 }) }
  try {
    const dto = UpdatePostDTO.parse(await request.json())
    const post = await BlogPostService.update(tenantId, postId, dto)
    return NextResponse.json({ post })
...
export async function DELETE(request: NextRequest, { params }: Ctx) {
  ...
  try { await auth(request, tenantId) } catch (e: any) { return NextResponse.json({ message: e.message }, { status: 403 }) }
  try {
    await BlogPostService.delete(tenantId, postId)
