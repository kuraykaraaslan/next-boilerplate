...
import { BackOfficeService, ApprovalActionDTO } from '@/modules/back_office';

/**
 * GET /tenant/[tenantId]/api/back-office/approvals/[approvalItemId]
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ item: await BackOfficeService.getApproval(tenantId, approvalItemId) }, { status: 200 });
  } catch (error: unknown) {
...
/**
 * PATCH /tenant/[tenantId]/api/back-office/approvals/[approvalItemId]
...
export async function PATCH(
  ...
  try {
    ...
    if (parsed.data.action === 'claim') {
      const item = await BackOfficeService.claim(tenantId, user.userId, approvalItemId);
      return NextResponse.json({ item }, { status: 200 });
    }

    const item = await BackOfficeService.decide(tenantId, user.userId, approvalItemId, {
