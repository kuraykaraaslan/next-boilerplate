...
import { BackOfficeService, ListApprovalsQuery, SubmitApprovalDTO } from '@/modules/back_office';

/**
 * GET /tenant/[tenantId]/api/back-office/approvals
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  try {
    ...
    return NextResponse.json(await BackOfficeService.listApprovals(tenantId, parsed.data), { status: 200 });
  } catch (error: unknown) {
...
/**
 * POST /tenant/[tenantId]/api/back-office/approvals
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  try {
    ...
    return NextResponse.json({ item: await BackOfficeService.submit(tenantId, parsed.data) }, { status: 201 });
  } catch (error: unknown) {
    const e = error as { message?: string; statusCode?: number };
    return NextResponse.json({ message: e.message }, { status: e.statusCode ?? 500 });
  }
