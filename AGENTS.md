...
|---|---|---|
| [modules/db](modules/db/) | `getDataSource()`, `tenantDataSourceFor(id)` (single DB by default; per-tenant DB override opt-in via `tenant_databases` row) | `import { getDataSource } from "@/modules/db";` |
...
3. **Register the entity** in the `ENTITIES` array in [modules/db/db.ts](modules/db/db.ts) — single DataSource holds all entities
